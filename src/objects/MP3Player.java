...
public class MP3Player extends BasicPlayer {
    ...
    public void play(String fileName) { //начала проигрывания файла
    ...
    public void stop() { //окончание проигрывания файла
        try {
            player.stop();
        } catch (BasicPlayerException ex) {
            Logger.getLogger(MP3Player.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void setVolume(int currentValue, int maximumValue) { //выставление громкости
        try {
        ...
        private double calcVolume(int currentValue, int maximumValue) { //вычисление текущей громкости 
        currentVolumeValue = (double) currentValue / (double) maximumValue;
        return currentVolumeValue;
    }

    public void jump(long bytes) { //перепрыгивание по песне
        try {
            player.seek(bytes);
            player.setGain(currentVolumeValue);
        } catch (BasicPlayerException ex) {
